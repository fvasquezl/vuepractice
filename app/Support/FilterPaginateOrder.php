        ...
        return $query->orderBy($request->column,$request->direction)
            ->where(function($query) use ($request){
                ...
                if($request->search_query_1) {
                    ...
                    //determine the type of column search_column
                    //check is related model, eg: customer.id
                    if ($this->isRelatedColumn($request)) {
                        ...
                        return $query->whereHas($relation,function($query) use ($relatedColumn,$request){
                            return $this->buildQuery($relatedColumn, $request->search_operator, $request, $query);
                        });
                    } else {
                        //regular column
    ...
    protected function buildQuery($column,$operator,$request,$query)
    {
        switch ($operator){
            ...
            case 'like':
                ...
                break;
            case 'between':
                $query->where($column,[$request->search_query_1,$request->search_query_1]);
                break;
            default:
